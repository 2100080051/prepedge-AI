...
export default function Login() {
  ...
  const handleGitHubLogin = async () => {
    ...
    if (!clientId) {
      setError('GitHub OAuth is not configured. Contact support.');
      setOauthLoading(null);
      return;
    }

    // Match the redirect_uri to exactly what is configured in GitHub API
    const redirectUri = 'https://www.prepedgeai.in/auth/github/callback';
        ...
        <div className="mx-auto w-full max-w-sm lg:w-96">
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            <button
              ...
              className="flex items-center justify-center gap-2 px-4 py-3 border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors font-medium text-sm text-slate-700 shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {oauthLoading === 'google' ? (
